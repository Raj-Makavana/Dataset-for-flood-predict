...
subprocess.run('git commit -m "Add Third Batch processing scripts and report"', shell=True)
...
for f in cleaned_files:
    ...
    if chunk_size >= 40 * 1024 * 1024:
        ...
        subprocess.run(f'git commit -m "Add Third Batch cleaned datasets part {chunk_idx}"', shell=True)
        subprocess.run("git push origin main", shell=True)
        chunk = []
        chunk_size = 0
        chunk_idx += 1
...
for f in raw_files:
    ...
    if chunk_size >= 40 * 1024 * 1024:
        ...
        cmd_add = "git add " + " ".join([f'"{c}"' for c in chunk])
        subprocess.run(cmd_add, shell=True)
        subprocess.run(f'git commit -m "Add Third Batch raw datasets part {chunk_idx}"', shell=True)
        subprocess.run("git push origin main", shell=True)
        chunk = []
        chunk_size = 0
        chunk_idx += 1
...
print("\n" + "=" * 90)
print("ALL THIRD BATCH DATASETS SUCCESSFULLY PUSHED TO GITHUB REPOSITORY!")
print("=" * 90)
